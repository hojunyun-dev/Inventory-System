...
@Slf4j
public class DanggeunRegistrationService {
    
    @Value("${platforms.danggeun.base-url}")
    private String danggeunBaseUrl;
    ...
    public ProductRegistration registerProduct(ProductRegistrationRequest request) {
        log.info("Registering product on Danggeun using automation: {}", request.getProductName());
        ...
        try {
            ...
            // Navigate to Danggeun login page
            driver.get(danggeunBaseUrl + "/login");
            
            // Login process
            loginToDanggeun(driver, wait);
            
            // Navigate to product registration page
            driver.get(danggeunBaseUrl + "/products");
            ...
            // Process response
            return processDanggeunResponse(request, driver);
            
        } catch (Exception e) {
            log.error("Failed to register product on Danggeun: {}", e.getMessage(), e);
            throw new RuntimeException("Danggeun registration failed: " + e.getMessage());
        } finally {
            if (driver != null) {
                driver.quit();
            }
    ...
    private void fillProductForm(WebDriver driver, WebDriverWait wait, ProductRegistrationRequest request) {
        ...
        try {
            // Product name
            WebElement nameField = wait.until(webDriver -> webDriver.findElement(By.name("title")));
            nameField.clear();
            nameField.sendKeys(request.getProductName());
            
            // Product description
            WebElement descriptionField = driver.findElement(By.name("content"));
    ...
    private ProductRegistration processDanggeunResponse(ProductRegistrationRequest request, WebDriver driver) {
        ProductRegistration registration = new ProductRegistration();
        registration.setPlatform("danggeun");

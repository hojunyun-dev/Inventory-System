        ...
        try {
            ...
            String hookingScript = """
                try {
                  ...
                  // 4) 로그아웃 버튼 클릭 감지 후킹
                  if (!window.__LOGOUT_HOOKED__) {
                    window.__LOGOUT_HOOKED__ = true;
                    
                    function detectLogoutClick() {
                      try {
                        // 다양한 로그아웃 버튼 셀렉터
                        const logoutSelectors = [
                          '//button[contains(text(), "로그아웃")]',
                          '//button[contains(text(), "로그아웃하기")]',
                          '//a[contains(text(), "로그아웃")]',
                          '//a[contains(text(), "로그아웃하기")]',
                        ...
                        function findLogoutButton() {
                          const xpath = "//button[contains(text(),'로그아웃')] | //a[contains(text(),'로그아웃')]";
                        ...
                        if (logoutBtn) {
                          // 기존 이벤트 리스너가 없으면 추가
                          if (!logoutBtn.__LOGOUT_LISTENER_ADDED__) {
                            logoutBtn.__LOGOUT_LISTENER_ADDED__ = true;
                            logoutBtn.addEventListener('click', function(e) {
                              try {
                                localStorage.setItem('__BUN_LOGOUT_FLAG__', 'true');
                                console.log('[LOGOUT] 로그아웃 버튼 클릭 감지됨 - 토큰 삭제 플래그 설정');
                              } catch (err) {
                                console.error('[LOGOUT] 플래그 설정 실패:', err);
                              }
                            }, true); // capture phase에서 먼저 실행
                            console.log('[LOGOUT] 로그아웃 버튼에 클릭 리스너 추가됨');
                          }
                        }
                      } catch (e) {
                        console.error('[LOGOUT] 로그아웃 버튼 감지 중 오류:', e);
                      }
                    }
                    ...
                    // MutationObserver로 DOM 변경 감지 (새로운 로그아웃 버튼 추가 시)
                    ...
                    if (document.readyState === 'loading') {
                      document.addEventListener('DOMContentLoaded', detectLogoutClick);
                    } else {
                      detectLogoutClick();
                    }
                    
                    // 주기적으로 확인 (3초마다)
                    setInterval(detectLogoutClick, 3000);
                    
                    console.log('[LOGOUT] 로그아웃 감지 후킹 스크립트 주입 완료');
                  }

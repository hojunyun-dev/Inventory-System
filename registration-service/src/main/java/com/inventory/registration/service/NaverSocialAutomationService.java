...
public class NaverSocialAutomationService {
    ...
    public AutomationResult registerProductToBunjangWithNaver(ProductData productData, String accessToken) {
        ...
        try {
            ...
            // 3. 네이버 OAuth 인증 처리
            if (!handleNaverOAuth(accessToken)) {
                errorMessage = "네이버 OAuth 인증 실패";
                screenshot = takeScreenshot("naver_oauth_failure");
            ...
            // 5. 상품 정보 입력
            if (!fillBunjangProductForm(productData)) {
                errorMessage = "번개장터 상품 폼 입력 실패";
                screenshot = takeScreenshot("bunjang_form_failure");
                throw new RuntimeException(errorMessage);
            }
            
            // 6. 상품 등록
            if (!submitBunjangProduct()) {
                errorMessage = "번개장터 상품 등록 실패";
                screenshot = takeScreenshot("bunjang_submit_failure");
                throw new RuntimeException(errorMessage);
            }
            
            // 7. 등록 성공 확인
            productUrl = verifyBunjangRegistration();
            if (productUrl == null) {
                errorMessage = "번개장터 등록 성공 확인 실패";
                screenshot = takeScreenshot("bunjang_verify_failure");
                throw new RuntimeException(errorMessage);
            }
            ...
            if (screenshot == null) {
                screenshot = takeScreenshot("bunjang_general_error");
            }
    ...
    public AutomationResult registerProductToJunggonaraWithNaver(ProductData productData, String accessToken) {
        ...
        try {
            ...
            // 6. 상품 정보 입력
            if (!fillJunggonaraProductForm(productData)) {
                errorMessage = "중고나라 상품 폼 입력 실패";
                screenshot = takeScreenshot("junggonara_form_failure");
                throw new RuntimeException(errorMessage);
            }
            
            // 7. 상품 등록
            if (!submitJunggonaraProduct()) {
                errorMessage = "중고나라 상품 등록 실패";
                screenshot = takeScreenshot("junggonara_submit_failure");
                throw new RuntimeException(errorMessage);
            }
            
            // 8. 등록 성공 확인
            productUrl = verifyJunggonaraRegistration();
            if (productUrl == null) {
                errorMessage = "중고나라 등록 성공 확인 실패";
                screenshot = takeScreenshot("junggonara_verify_failure");
                throw new RuntimeException(errorMessage);
            }
            ...
            if (screenshot == null) {
                screenshot = takeScreenshot("junggonara_general_error");
            }
            if (errorMessage == null) {
                errorMessage = e.getMessage();
            }
     ...
     * 네이버 OAuth 인증 처리
     */
    private boolean handleNaverOAuth(String accessToken) {
        try {
     ...
     * 네이버 로그인 상태 확인
     */
    private boolean isNaverLoggedIn() {
        try {
     ...
     * 번개장터 상품 폼 입력
     */
    private boolean fillBunjangProductForm(ProductData productData) {
        try {
            WebDriverWait wait = new WebDriverWait(webDriver, Duration.ofSeconds(10));
     ...
     * 번개장터 상품 등록
     */
    private boolean submitBunjangProduct() {
        try {
     ...
     * 번개장터 등록 성공 확인
     */
    private String verifyBunjangRegistration() {
        try {
     ...
     * 중고나라 상품 폼 입력
     */
    private boolean fillJunggonaraProductForm(ProductData productData) {
        try {
            WebDriverWait wait = new WebDriverWait(webDriver, Duration.ofSeconds(10));
     ...
     * 중고나라 상품 등록
     */
    private boolean submitJunggonaraProduct() {
        try {
            WebElement submitButton = webDriver.findElement(By.cssSelector("button[type='submit']"));
            submitButton.click();
     ...
     * 중고나라 등록 성공 확인
     */
    private String verifyJunggonaraRegistration() {
        try {
            String currentUrl = webDriver.getCurrentUrl();
     ...
     * 스크린샷 촬영
     */
    private String takeScreenshot(String fileNamePrefix) {
        try {

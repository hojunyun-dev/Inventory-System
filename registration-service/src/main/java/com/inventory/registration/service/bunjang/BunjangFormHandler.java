...
public class BunjangFormHandler {
    ...
    public void fillProductForm(WebDriver driver, WebDriverWait wait, ProductRegistrationRequest request) {
        ...
        while (retryCount < maxRetries) {
            try {
                ...
                // 1. 이미지 업로드 (최우선)
                uploadProductImages(driver, request);
     ...
     * 상품 이미지 업로드
     */
    private void uploadProductImages(WebDriver driver, ProductRegistrationRequest request) {
        log.info("📸 상품 이미지 업로드 시작...");
        
        try {
            // 재고관리 시스템에서 이미지 가져오기
            String[] imageUrls = request.getImages();
            if (imageUrls == null || imageUrls.length == 0) {
                log.warn("⚠️ 업로드할 이미지가 없습니다.");
                return;
            }
            ...
            WebElement fileInput = findElementBySelectors(driver, uploadSelectors);
            
            // 이미지 업로드 (최대 12개)
            int maxImages = Math.min(12, imageUrls.length);
            for (int i = 0; i < maxImages; i++) {
                ...
                if (imageUrl != null && !imageUrl.trim().isEmpty()) {
                    ...
                    if (new java.io.File(absolutePath).exists()) {
                        ...
                        log.info("✅ 이미지 업로드 완료: {}", absolutePath);
                        Thread.sleep(1000); // 업로드 대기
                    } else {
                        log.warn("⚠️ 이미지 파일을 찾을 수 없음: {}", absolutePath);
                    }
                }
            }
            
            log.info("✅ 이미지 업로드 완료: {}개", maxImages);

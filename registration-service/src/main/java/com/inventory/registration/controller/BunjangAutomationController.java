...
public class BunjangAutomationController {
    ...
    // 프론트 호환용 별칭: /api/automation/platform/bunjang/session/open
    @PostMapping("/platform/bunjang/session/open")
    public ResponseEntity<Map<String, Object>> openManualLoginWindowAlias() {
        return openManualLoginWindow();
    }
    ...
    // 프론트 호환용 별칭: /api/automation/platform/bunjang/session/open-with-product
    @PostMapping("/platform/bunjang/session/open-with-product")
    public ResponseEntity<Map<String, Object>> openManualLoginWithProductAlias(@RequestBody Map<String, Object> request) {
        return openManualLoginWithProduct(request);
    }
    ...
    public ResponseEntity<Map<String, Object>> sessionStatus() {
        try {
            // WebDriver를 기동하지 않는 경량 상태 확인으로 500을 방지
            Map<String, Object> result = bunjangRegistrationService.checkLoginStatusLight();
            return ResponseEntity.ok(result);
        } catch (Exception e) {
            log.error("로그인 상태 확인 실패: {}", e.getMessage());
            // 프론트에서 500 대신 payload로 실패를 처리할 수 있도록 200으로 내려줌
            return ResponseEntity.ok(Map.of(
                "success", false,
                "loggedIn", false,
    ...
    // 프론트에서 사용하는 대체 경로 호환: /api/automation/platform/bunjang/register
    @PostMapping(path = "/platform/bunjang/register")
    public ResponseEntity<Map<String, Object>> registerProductAlias(@RequestBody Map<String, Object> request) {

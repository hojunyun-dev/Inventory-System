...
async function withRetry<T>(fn: () => Promise<T>, attempts = 3, delayMs = 400): Promise<T> {
  ...
  for (let i = 0; i < attempts; i++) {
    try {
      ...
      if (code === 'ERR_NETWORK' || code === 'ECONNABORTED' || code === 'ECONNREFUSED' || code === 502 || code === 503) {
        if (i < attempts - 1) await new Promise(r => setTimeout(r, delayMs * Math.pow(1.5, i)));
        continue;
      }
      throw err;
    }
...
export const bunjangApi = {
  checkLoginStatus: () => withRetry(() => bunjangApiClient.get('/automation/bunjang/session/status')),
  openForManualLogin: () => bunjangApiClient.post('/automation/platform/bunjang/session/open'),
  openWithProduct: (productData: {
    ...
    quantity: number;
    category: string;
  }) => bunjangApiClient.post('/automation/platform/bunjang/session/open-with-product', productData),
